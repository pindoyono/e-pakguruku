      ...
      <div class="col-md-12">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <form enctype="multipart/form-data" class="form-horizontal"  action="{{route('l2pkb.store')}}" method="POST">
                @csrf
                <div class="row">
                    <label class="col-sm-2 col-form-label">Usulan yg di Tolak</label>
                    <div class="col-sm-10">
                        <div class="form-group">
                            <select name="pendidikan_id" data-style="select-with-transition" class="selectpicker" title="Pilih Usulan Yg Perlu Perbaiakan" data-live-search="true">
                                @foreach ($usulan as $item)
                                <option  value="{{ $item->id }}">{{ Str::limit('['.$item->id.']'.$item->kegiatan) }}</option>
                                @endforeach
                              </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-10">
                        <div class="form-group">
                            <select name="l2pkb_id" data-style="select-with-transition" class="selectpicker" title="Pilih Alasan dan Saran" data-live-search="true">
                                @foreach ($lampiran as $item)
                                <option  value="{{ $item->id }}">{{ Str::limit('['.$item->kode.']'.$item->diskripsi) }}</option>
                                @endforeach
                              </select>
                        </div>
                    </div>
