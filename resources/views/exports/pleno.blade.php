...
<body>
    <table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
        ...
        <tbody>
          ...
          <tr>
            <td>{{ $i++ }}</td>
            <td>{{ $pak->penilai_id!=null?get_data_penilai( $pak->penilai_id)->name:'Belum Di Nilai'  }}</td>
            <td>{{ $pak->name }}</td>
            <td>{{ $pak->pangkat }}</td>
            ...
            <td >{{ $pak->target }}</td>
            <td style="{{  number_format($ak_diperoleh - $pak->target,3) >= 0 ? 'color: green;' : 'color: red;'  }} text-align:right; font-weight:bold">{{ str_replace('.',',', number_format($ak_diperoleh - $pak->target,3)) }}</td>
            ...
            <td>
                {{
                    ...
                    $pak->publikasi_ilmiah + $pak->publikasi_ilmiah2 +
                    $pak->karya_inovatif + $pak->karya_inovatif2
                    - (90/100*$pak->target_sebelum) + ($pak->tertinggal + $pak->tertinggal2 + $pak->tertinggal_user)
                    ,3);
                }}
            </td>
            <td >
            ...
            <td style="{{ number_format( $ak_utama_total - (90/100*$pak->akk),3) >=0 ? 'color: green;' : 'color: red;'  }} text-align:right;font-weight:bold">{{ str_replace('.',',', number_format( $ak_utama_total - (90/100*$pak->akk),3) ) }}</td>
            ...
            <td >
                {{
                    str_replace('.',',',
                       $ak_pd = number_format(
                        $pak->pengembangan_diri + $pak->pengembangan_diri2 - $pak->pd_user
                        ,3));
                }}
            </td>
            <td >
            ...
            <td style="{{ number_format($ak_pd - $pak->akpkbpd,3)>=0 ? 'color: green;' : 'color: red;'  }} text-align:right;font-weight:bold">{{ str_replace('.',',',number_format($ak_pd - $pak->akpkbpd,3)) }}</td>
            ...
            <td>
                {{
                   ...
                   $ak_piki = number_format(
                        $pak->publikasi_ilmiah + $pak->publikasi_ilmiah2 +
                        $pak->karya_inovatif + $pak->karya_inovatif2 - $pak->pi_user
                        - $pak->ki_user
                    ,3));
                }}
            </td>
            ...
            @if (get_periode()==4)
                <td style="{{ masa_kerja(\Carbon\Carbon::parse((date("Y")+1)."-04-01"), $pak->tmt_pns) >= 2 ? 'color: green;' : 'color: red;'  }}" >{{ masa_kerja(\Carbon\Carbon::parse((date("Y")+1)."-04-01"), $pak->tmt_pns)  }} </td>
            ...
            @if ($pak->jabatan_id >=4 )
            <td style="{{$pak->karya_inovatif + $pak->karya_inovatif2 - $pak->ki_user <= 50/100*$pak->akpkbpiki ? 'color: green;' : 'color: red;'  }}" > Perolehan ({{$pak->karya_inovatif + $pak->karya_inovatif2 - $pak->ki_user}})   </td>
            @else
            ...
            <td style="{{ lolos($pak->pak_id) == "Lolos"  ? 'color: green;' : 'color: red;'  }}" > {{ lolos($pak->pak_id) }}</td>
            @if (lolos($pak->pak_id) != "Lolos")
            <td><a target="_blank" href="{{ lolos($pak->pak_id) != "Lolos"  ? route('penilais.cetak_hapak',$pak->pak_id)  : ''  }}">Hapak</a> <td>
            @else
